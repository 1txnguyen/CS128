...
void AddStudent(OfficeHoursQueue& queue, const Student& student) {
  ...
  int i = queue.student_queue.size() - 1;
  while (i > 0) {
    if (queue.student_queue[i].attendance_percentage >
        queue.student_queue[i - 1].attendance_percentage) {
      Student temp = queue.student_queue[i - 1];
      queue.student_queue[i - 1] = queue.student_queue[i];
      queue.student_queue[i] = temp;

    } else if (queue.student_queue[i].attendance_percentage ==
               queue.student_queue[i - 1].attendance_percentage) {
      if (queue.student_queue[i].arrival_order <
          queue.student_queue[i - 1].arrival_order) {
        Student temp = queue.student_queue[i - 1];
        queue.student_queue[i - 1] = queue.student_queue[i];
        queue.student_queue[i] = temp;
      }
